...
function ActionNode({ data }: NodeProps<BTFlowNodeData>) {
  ...
  const nodeContent = (
    <div
      className="relative px-3 py-2 rounded border-2 bg-emerald-900/40 backdrop-blur-sm transition-all duration-300"
      style={{
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-emerald-100 text-sm font-medium truncate">
          {data.nodeName}
        </span>
      </div>

      {data.note && (
        <div className="text-xs text-emerald-300/70 mt-1 truncate">{data.note}</div>
      )}
